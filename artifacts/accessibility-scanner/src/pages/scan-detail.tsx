...
function IssueFilterBar({
  ...
  const wcagCriteria = useMemo(
    () =>
      Array.from(
        new Set(issues.map((i) => i.wcagCriteria).filter(Boolean) as string[]),
      ).sort(),
    [issues],
  );
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-foreground/80 break-words">
                    {info?.description ?? "No issues detected for this rule on this page."}
                  </p>
...
export default function ScanDetail() {
  ...
  const handleRetry = () => {
    if (!scan) return;
    const urls = (scan.pages ?? [])
      .filter(
        (p: { url: string; status: string }) =>
    ...
    if (urls.length === 0) {
      toast({
        ...
        description: "All pages completed successfully — no failed or pending URLs.",
        variant: "destructive",
      });
      return;
    }
    ...
    retryScan.mutate(
      {
        data: {
          urls,
          name: scan.name ? `${scan.name} (retry failed)` : undefined,
          options: {
            maxConcurrency: (opts.maxConcurrency as number) ?? 5,
            ...(Array.isArray(opts.rules) && opts.rules.length > 0
              ? { rules: opts.rules as string[] }
              : {}),
            ...(opts.proxyPacUrl
              ? { proxyPacUrl: opts.proxyPacUrl as string }
              : {}),
          },
        },
      },
