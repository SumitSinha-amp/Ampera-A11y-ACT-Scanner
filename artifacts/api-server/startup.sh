...
if grep -q "projectSitesTable3" "$SERVER_BUNDLE" 2>/dev/null; then
  echo "=== FATAL: stale API bundle detected (projectSitesTable3) ===" >&2
  echo "=== Rebuild and redeploy the current artifacts/api-server/dist/index.mjs ===" >&2
  exit 1
fi
